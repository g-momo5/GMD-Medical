<script lang="ts">
  ...
  export let anamnesi_cardiologica = '';
  ...
  function handleKeyDown(event: KeyboardEvent) {
    ...
    const textBeforeCursor = anamnesi_cardiologica.substring(0, cursorPos);
    const textAfterCursor = anamnesi_cardiologica.substring(cursorPos);
      ...
      if (bulletMatch) {
        ...
        if (event.shiftKey) {
          ...
          if (currentIndent.length >= 2) {
            ...
            anamnesi_cardiologica = beforeLastLine + newLine + textAfterCursor;
        ...
        anamnesi_cardiologica = textBeforeCursor + '  ' + textAfterCursor;
        setTimeout(() => {
      ...
      if (bulletMatch) {
        ...
        if (lastLine.trim() === bullet.trim()) {
          ...
          if (currentIndent.length >= 2) {
            ...
            anamnesi_cardiologica = textBeforeCursor.substring(0, textBeforeCursor.length - bullet.length) + '\n' + textAfterCursor;
          ...
          anamnesi_cardiologica = textBeforeCursor + '\n' + bullet + textAfterCursor;
...
<Card>
  <h2 class="section-title">Anamnesi Patologica Remota</h2>
  ...
  <div class="anamnesi-content">
    ...
    <RichTextarea
      ...
      id="anamnesi_cardiologica"
      bind:value={anamnesi_cardiologica}
      on:keydown={handleKeyDown}
      on:keyup={handleSelectionChange}
      on:click={handleSelectionChange}
      placeholder="Inserisci l'anamnesi cardiologica del paziente... Usa i pulsanti per formattare il testo."
      rows={6}
    />
  </div>
</Card>
