<script lang="ts">
  ...
  export let terapia_domiciliare = '';
  ...
  function handleKeyDown(event: KeyboardEvent) {
    ...
    const textBeforeCursor = terapia_domiciliare.substring(0, cursorPos);
    const textAfterCursor = terapia_domiciliare.substring(cursorPos);
      ...
      if (bulletMatch) {
        ...
        if (event.shiftKey) {
          ...
          if (currentIndent.length >= 2) {
            ...
            terapia_domiciliare = beforeLastLine + newLine + textAfterCursor;
        ...
        terapia_domiciliare = textBeforeCursor + '  ' + textAfterCursor;
        setTimeout(() => {
      ...
      if (bulletMatch) {
        ...
        if (lastLine.trim() === bullet.trim()) {
          ...
          if (currentIndent.length >= 2) {
            ...
            terapia_domiciliare = textBeforeCursor.substring(0, textBeforeCursor.length - bullet.length) + '\n' + textAfterCursor;
          ...
          terapia_domiciliare = textBeforeCursor + '\n' + bullet + textAfterCursor;
...
<Card>
  <h2 class="section-title">Terapia Domiciliare</h2>
  ...
  <div class="anamnesi-content">
    ...
    <RichTextarea
      ...
      bind:value={terapia_domiciliare}
      on:keydown={handleKeyDown}
      on:keyup={handleSelectionChange}
      on:click={handleSelectionChange}
      placeholder="Inserisci la terapia domiciliare del paziente... Usa i pulsanti per formattare il testo."
      rows={6}
    />
  </div>
</Card>
